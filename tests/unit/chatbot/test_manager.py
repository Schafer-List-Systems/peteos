...
class TestChatBotManagerAddBackend:
    ...
    async def test_add_backend_openai(self):
        ...
        with patch("peteos.chatbot.manager.HTTPClient", side_effect=mock_factory):
    ...
    async def test_add_backend_anthropic(self):
        ...
        mock_response = {"models": [{"id": "anthropic-model-1"}, {"id": "anthropic-model-2"}]}
        mock_factory = _make_http_client_mock(return_value=mock_response)
    ...
    @pytest.mark.asyncio
    async def test_add_backend_explicit_api_type(self):
    ...
    @pytest.mark.asyncio
    async def test_add_backend_invalid_api_type(self):
    ...
    async def test_add_backend_unrecognized_format(self):
        """Adding backend with unrecognized response format raises error."""
...
class TestChatBotManagerLoadFromJson:
    ...
    async def test_load_from_json_openai(self):
        ...
        mock_response = {"data": [{"id": "local-model-1"}, {"id": "local-model-2"}]}
        mock_factory = _make_http_client_mock(return_value=mock_response)
    ...
    async def test_load_from_json_anthropic(self):
        ...
        mock_response = {"models": [{"id": "claude-3-opus"}]}
        mock_factory = _make_http_client_mock(return_value=mock_response)
    ...
    async def test_load_from_json_clears_existing(self):
        ...
        with patch("peteos.chatbot.manager.HTTPClient", side_effect=new_mock_factory):
            await ChatBotManager.load_from_json(json_obj)
...
class TestChatBotManagerReset:
    ...
    @pytest.mark.asyncio
    async def test_reset_allows_reload(self):
...
class TestChatBotManagerInit:
    """Tests for __init__ method."""

    @pytest.mark.asyncio
    async def test_init_sets_timeout(self):
        """Instantiating ChatBotManager sets the class-level timeout."""
        mock_response = {"data": [{"id": "model-1"}]}
        mock_factory = _make_http_client_mock(return_value=mock_response)

        manager = ChatBotManager(timeout=60.0)
        assert ChatBotManager._timeout == 60.0

    @pytest.mark.asyncio
    async def test_init_none_keeps_existing(self):
