...
class TestGeminiToolResultMatching:
    ...
    def test_tool_result_matches_tool_use_from_model_response(self, chatbot):
        """tool_result should find the function name from the preceding model message's tool_use part."""
        ...
        context.messages = [model_msg, tool_result_msg]
    ...
    def test_tool_result_falls_back_to_unknown(self, chatbot):
        """If no preceding model message with functionCall is found, use 'unknown'."""
        tool_result_msg = Message.create(
            role="tool_result",
            content_parts=[
        ...
        user_msgs = [m for m in body["contents"] if m["role"] == "user"]
        assert len(user_msgs) == 1
        assert user_msgs[0]["parts"][0]["functionResponse"]["name"] == "unknown"
    ...
    def test_multiple_tool_results_in_sequence(self, chatbot):
        ...
        context = MagicMock(spec=Context)
        context.messages = [model_msg, tool_result_a, tool_result_b]

        body = chatbot._build_body(context)

        # Should have: 1 model message, 1 user message (all functionResponses merged)
        model_msgs = [m for m in body["contents"] if m["role"] == "model"]
        user_msgs = [m for m in body["contents"] if m["role"] == "user"]
    ...
    def test_full_conversation_roundtrip(self, chatbot):
        ...
        model_msg2 = Message.create(
            role="model",
            content_parts=[ContentPart.create_text("The answer is 5.")],
        )
        context = MagicMock(spec=Context)
        context.messages = [user_msg, model_msg, tool_result_msg, model_msg2]
